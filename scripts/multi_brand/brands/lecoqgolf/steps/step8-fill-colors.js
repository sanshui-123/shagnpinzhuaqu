...
const step8 = async (ctx) => {
  ...
  try {
    ...
    // 如果颜色数量过多，删除多余颜色
    while (existingColors.length > colorCount && colorCount > 0) {
      const deleteButtons = await page.$$('.color-item .btn-delete, [data-sku-type="color"] .delete-btn');
      if (deleteButtons.length > 0) {
        await deleteButtons[deleteButtons.length - 1].click();
        ctx.logger.info(`删除多余颜色 ${existingColors.length}/${colorCount}`);
        await page.waitForTimeout(500);
        existingColors.pop();
      } else {
        break;
    ...
    // 检查是否有图片上传功能
    const imageUploadButtons = await page.$$('[data-sku-type="color"] .upload-btn, .color-image-upload');
    if (imageUploadButtons.length > 0) {
      ctx.logger.info(`找到 ${imageUploadButtons.length} 个颜色图片上传按钮`);

      // 为每个颜色上传对应的图片
      for (let i = 0; i < Math.min(colors.length, colorCount); i++) {
        const colorImages = taskCache.productData.colorImages[i] || [];
        if (colorImages.length > 0) {
          ctx.logger.info(`  颜色${i + 1}需要上传 ${colorImages.length} 张图片`);
          // 这里可以实现图片上传逻辑
          // 暂时跳过，因为图片已在步骤5中上传
        }
      }
    ...
    // 检查是否有错误提示
    const errorMessages = await page.$$('[data-field="color"] .error-message, .color-error');
    if (errorMessages.length > 0) {
      ctx.logger.warn('发现颜色配置错误:');
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${ctx.productId}_step8_colors.png`
    );
    ...
    taskCache.stepStatus[8] = 'done';
    saveTaskCache(ctx.productId, taskCache);
    updateStepStatus(ctx.productId, 8, 'done');
    ...
    if (ctx.page1) {
      try {
        const errorScreenshot = path.join(
          path.resolve(process.cwd(), 'screenshots'),
          `${ctx.productId}_step8_error.png`
        );
    ...
    updateStepStatus(ctx.productId, 8, 'failed');
    throw error;
