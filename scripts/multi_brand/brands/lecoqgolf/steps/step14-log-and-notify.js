...
const step14 = async (ctx) => {
  ...
  try {
    ...
    const notification = {
      type: report.summary.status === 'success' ? 'success' : 'error',
      productId: productId,
      title: taskCache.productData?.titleJP || taskCache.productData?.titleCN || '',
      duration: formatDuration(duration),
      summary: report.summary,
      taobaoUrl: taskCache.submitResults?.taobaoUrl || null
    };

    // 发送通知（异步，不阻塞）
    notificationService.send(notification).catch(error => {
      ctx.logger.warn(`发送通知失败: ${error.message}`);
    });
    ...
    updateStepStatus(productId, 14, 'done');
    ...
    updateStepStatus(ctx.productId, 14, 'failed');
    throw error;
  }
};
