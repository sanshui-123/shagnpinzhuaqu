...
class HonmaScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Honma Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `honma_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new HonmaScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Honma Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Honma Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = HonmaScraper;
