...
class ClevelandScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Cleveland Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `cleveland_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new ClevelandScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Cleveland Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Cleveland Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = ClevelandScraper;
