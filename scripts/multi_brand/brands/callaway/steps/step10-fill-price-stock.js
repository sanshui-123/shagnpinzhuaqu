...
const step10 = async (ctx) => {
  ...
  try {
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${ctx.productId}_step10_price_stock.png`
    );
    ...
    taskCache.stepStatus[10] = 'done';
    saveTaskCache(ctx.productId, taskCache);
    updateStepStatus(ctx.productId, 10, 'done');
    ...
    if (ctx.page1) {
      try {
        const errorScreenshot = path.join(
          path.resolve(process.cwd(), 'screenshots'),
          `${ctx.productId}_step10_error.png`
        );
    ...
    updateStepStatus(ctx.productId, 10, 'failed');
    throw error;
