    ...
    try {
      const result = await page.evaluate(() => {
        ...
        for (const el of allClickable) {
          ...
          const text = (el.innerText || el.textContent || el.value || '').toLowerCase().trim();
          // 严格限制：文本长度2-20字符（排除长文本按钮如HELP菜单）
          if (text.length < 2 || text.length > 20) continue;

          // 排除包含导航/帮助关键词的按钮
          const excludeKeywords = ['help', 'faq', 'menu', 'メニュー', 'ヘルプ', '問い合わせ', 'お電話'];
          const hasExcludeWord = excludeKeywords.some(keyword => text.includes(keyword));
...
async function scrapeCategory(url, category) {
  ...
  try {
    ...
    if (!expectedCount) {
      console.log('⚠️ 未找到页面显示的商品数量，使用默认值继续执行...');
      // Callaway 页面可能不显示商品总数，使用默认值
      expectedCount = 100; // 默认预期抓取100个商品
      console.log(`使用默认预期数量: ${expectedCount}`);
    }
