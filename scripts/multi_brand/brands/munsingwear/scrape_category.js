...
class MunsingwearScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Penguin by Munsingwear 数据...');
            ...
            while (currentPage <= maxPages) {
                ...
                return Array.from(items).map((item, index) => {
                    try {
                        ...
                        const brand = brandElement ? brandElement.textContent.trim() : 'Penguin by Munsingwear';
    ...
    async saveResults() {
        try {
            ...
            const outputFile = path.join(outputPath, `munsingwear_products_${timestamp}.json`);

            const totalProducts = this.results.reduce((sum, r) => sum + (r.products?.length || 0), 0);

            const outputData = {
                brand: 'Penguin by Munsingwear',
                brandId: 'munsingwear',
...
if (require.main === module) {
    ...
    try {
        ...
        const scraper = new MunsingwearScraper(config);

        scraper.scrape()
            .then(() => scraper.saveResults())
            .then(() => console.log('✅ Penguin by Munsingwear 抓取完成'))
            .catch(error => {
                console.error('❌ Penguin by Munsingwear 抓取失败:', error);
                process.exit(1);
            });
...
module.exports = MunsingwearScraper;
