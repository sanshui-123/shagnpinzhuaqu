...
const step13 = async (ctx) => {
  ...
  try {
    ...
    const incompleteSteps = requiredSteps.filter(step => taskCache.stepStatus[step] !== 'done');
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${ctx.productId}_step13_submit.png`
    );
    ...
    taskCache.stepStatus[13] = submitResult.status === 'success' ? 'done' : 'failed';
    saveTaskCache(ctx.productId, taskCache);
    updateStepStatus(ctx.productId, 13, taskCache.stepStatus[13]);
    ...
    if (ctx.page1) {
      try {
        const errorScreenshot = path.join(
          path.resolve(process.cwd(), 'screenshots'),
          `${ctx.productId}_step13_error.png`
        );
    ...
    updateStepStatus(ctx.productId, 13, 'failed');
    throw error;
