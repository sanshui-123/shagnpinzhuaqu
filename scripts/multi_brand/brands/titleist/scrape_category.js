...
class TitleistScraper {
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `titleist_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new TitleistScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Titleist 抓取完成'))
        .catch(error => {
            console.error('❌ Titleist 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = TitleistScraper;
