...
class OdysseyScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Odyssey Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `odyssey_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new OdysseyScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Odyssey Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Odyssey Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = OdysseyScraper;
