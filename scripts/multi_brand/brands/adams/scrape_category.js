...
class AdamsScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Adams Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `adams_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new AdamsScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Adams Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Adams Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = AdamsScraper;
