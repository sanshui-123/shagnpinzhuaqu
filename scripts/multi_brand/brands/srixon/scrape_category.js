...
class SrixonScraper {
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `srixon_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new SrixonScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Srixon 抓取完成'))
        .catch(error => {
            console.error('❌ Srixon 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = SrixonScraper;
