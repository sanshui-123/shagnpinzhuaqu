...
class MizunoScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Mizuno Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `mizuno_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new MizunoScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Mizuno Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Mizuno Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = MizunoScraper;
