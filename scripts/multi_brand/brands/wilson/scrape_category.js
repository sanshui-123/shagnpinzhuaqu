...
class WilsonScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Wilson Staff 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `wilson_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new WilsonScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Wilson Staff 抓取完成'))
        .catch(error => {
            console.error('❌ Wilson Staff 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = WilsonScraper;
