...
class PxgScraper {
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `pxg_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new PxgScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ PXG 抓取完成'))
        .catch(error => {
            console.error('❌ PXG 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = PxgScraper;
