...
class PingScraper {
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `ping_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new PingScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ PING Golf 抓取完成'))
        .catch(error => {
            console.error('❌ PING Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = PingScraper;
