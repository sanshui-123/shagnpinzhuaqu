...
class ScottyScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Scotty Cameron 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `scotty_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new ScottyScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Scotty Cameron 抓取完成'))
        .catch(error => {
            console.error('❌ Scotty Cameron 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = ScottyScraper;
