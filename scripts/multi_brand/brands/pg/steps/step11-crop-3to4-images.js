...
const step11 = async (ctx) => {
  ...
  try {
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${ctx.productId}_step11_cropped.png`
    );
    ...
    taskCache.stepStatus[11] = 'done';
    saveTaskCache(ctx.productId, taskCache);
    updateStepStatus(ctx.productId, 11, 'done');
    ...
    if (ctx.page1) {
      try {
        const errorScreenshot = path.join(
          path.resolve(process.cwd(), 'screenshots'),
          `${ctx.productId}_step11_error.png`
        );
    ...
    updateStepStatus(ctx.productId, 11, 'failed');
    throw error;
