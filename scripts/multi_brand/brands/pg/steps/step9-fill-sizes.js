...
const step9 = async (ctx) => {
  ...
  try {
    ...
    // 步骤7：填写SKU相关信息（可选）
    if (productData.skuInfos && productData.skuInfos.length > 0) {
      ctx.logger.info('\n[步骤5] 填写SKU额外信息');
      await fillSKUAdditionalInfo(page, productData.skuInfos, ctx);
    }
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${ctx.productId}_step9_sizes.png`
    );
    ...
    taskCache.stepStatus[9] = 'done';
    saveTaskCache(ctx.productId, taskCache);
    updateStepStatus(ctx.productId, 9, 'done');
    ...
    if (ctx.page1) {
      try {
        const errorScreenshot = path.join(
          path.resolve(process.cwd(), 'screenshots'),
          `${ctx.productId}_step9_error.png`
        );
    ...
    updateStepStatus(ctx.productId, 9, 'failed');
    throw error;
 ...
 * 填写SKU额外信息
 */
async function fillSKUAdditionalInfo(page, skuInfos, ctx) {
  for (const skuInfo of skuInfos) {
    try {
      // 查找对应的SKU行
      const skuSelector = `[data-sku-id="${skuInfo.skuId}"], [data-color="${skuInfo.color}"][data-size="${skuInfo.size}"]`;
      const skuRow = await page.$(skuSelector);

      if (skuRow) {
        // 填写SKU编码
        if (skuInfo.skuCode) {
          const codeInput = await skuRow.$('input[placeholder*="SKU"], input[name*="sku"]');
          if (codeInput) {
            await codeInput.fill(skuInfo.skuCode);
          }
        }

        // 填写商家编码
        if (skuInfo.merchantCode) {
          const merchantInput = await skuRow.$('input[placeholder*="商家编码"]');
          if (merchantInput) {
            await merchantInput.fill(skuInfo.merchantCode);
          }
        }
      }
    } catch (error) {
      ctx.logger.warn(`填写SKU额外信息失败: ${error.message}`);
    }
  }
}
