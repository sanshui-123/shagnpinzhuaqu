...
class TaylormadeScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 TaylorMade Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `taylormade_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new TaylormadeScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ TaylorMade Golf 抓取完成'))
        .catch(error => {
            console.error('❌ TaylorMade Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = TaylormadeScraper;
