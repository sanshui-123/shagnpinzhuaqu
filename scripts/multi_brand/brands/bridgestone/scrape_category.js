...
class BridgestoneScraper {
    ...
    async scrape() {
        console.log('🚀 开始抓取 Bridgestone Golf 数据...');
    ...
    async saveResults() {
        ...
        const outputFile = path.join(outputPath, `bridgestone_products_${timestamp}.json`);
...
if (require.main === module) {
    ...
    const scraper = new BridgestoneScraper(config);

    scraper.scrape()
        .then(() => scraper.saveResults())
        .then(() => console.log('✅ Bridgestone Golf 抓取完成'))
        .catch(error => {
            console.error('❌ Bridgestone Golf 抓取失败:', error);
            process.exit(1);
        });
}

module.exports = BridgestoneScraper;
