...
build_smart_prompt 函数（360-430行）- 构建提示词
generate_cn_title 函数（634-740行）- 生成标题主流程
call_glm_api 函数（236-353行）- 调用GLM API
validate_title_quality 函数（558-658行）- 验证标题质量
    Returns:
        "男" or "女"
    """
...
- 26春夏卡拉威高尔夫女士时尚运动腰带（27字）
- 25秋冬卡拉威高尔夫男士球杆头套（24字）← 如果太短，加"轻便"或"新款"
- 25秋冬卡拉威高尔夫男士专业高尔夫手套（27字）

服装类示例：
- 25秋冬卡拉威高尔夫男士保暖舒适夹克（27字）
- 26春夏卡拉威高尔夫女士弹力全拉链上衣（28字）
- 25秋冬卡拉威高尔夫男士保暖棉服夹克（27字）
- 26春夏卡拉威高尔夫女士轻便运动短裤（28字）
...
def optimize_title(title: str) -> str:
    ...
    if len(title) > 30:
        ...
        # 如果太短，尝试在适当位置加修饰词
        modifiers = ['新款', '时尚', '轻便', '透气', '运动', '专业', '经典']
        ...
        if insert_pos > 0:
            for mod in modifiers:
                test_title = title[:insert_pos] + mod + title[insert_pos:]
    ...
    for attempt in range(3):
        ...
        if raw_title:
            ...
            if validate_title(title, product):
                ...
                # 如果验证失败，记录失败原因
                print(f"尝试 {attempt + 1} 失败: 标题验证不通过 - {title[:50]}")

                # 如果是前两次失败，调整提示词重试
                if attempt < 2:
                    # 调整提示词，强调常见错误
                    prompt += "\n\n重要提醒：请直接输出26-30字的标题，不要解释，确保'高尔夫'只出现1次！"
        else:
            print(f"尝试 {attempt + 1} 失败: GLM返回空")

    # 如果3次都失败，返回空字符串（让调用方处理）
    print("❌ GLM生成失败，3次尝试未通过验证")
    return ""
