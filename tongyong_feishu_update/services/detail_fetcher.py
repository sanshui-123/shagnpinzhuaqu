...
class DetailFetcher:
    ...
    def needs_detail_fetch(self, product: Dict) -> bool:
        ...
        # 检查关键字段，支持多种字段名格式和数据结构
        colors = product.get('colors', [])
        images = product.get('imageUrls', []) or product.get('imagesMetadata', [])

        # 如果有品牌、有产品链接、有颜色、有图片，就不需要重新抓取
        has_brand = bool(product.get('brand', ''))
        has_url = bool(product.get('detailUrl', ''))
        has_colors = bool(colors)
        has_images = bool(images)

        # 对于包类产品，没有尺码是正常的，不强制要求
        # 只有在缺少关键信息时才抓取
        return not (has_brand and has_url and has_colors and has_images)
